...
function renderStats() {
  ...
  const itemsHtml = personItems.map(function(item) {
    ...
    return '<div class="stats-item-row">' +
      ...
      '<div class="stats-item-type' + (item.type && item.type.includes('超大型') ? ' stats-item-type-xlarge' : item.type && item.type.includes('大型') ? ' stats-item-type-large' : item.type && item.type.includes('中型') ? ' stats-item-type-medium' : item.type && item.type.includes('小型') ? ' stats-item-type-small' : '') + '">' + escHtml(item.type || '') + '</div>' +
